...
const TopPanel = (props: propsInterface) => {
    ...
    const getDaysAgo = (start: Date): string => {
        ...
        if (yearsDiff > 0) {
            result += yearsDiff === 1 ? `1 ${language.dates.year}` : `${Math.floor(yearsDiff)} ${language.dates.years}`;
        }
        ...
        if (remainingMonths > 0) {
            ...
            result += remainingMonths === 1 ? `1 ${language.dates.month}` : `${Math.floor(remainingMonths)} ${language.dates.months}`;
        }
        ...
        if (remainingDays > 0) {
            ...
            result += remainingDays === 1 ? `1 ${language.dates.day}` : `${Math.floor(remainingDays)} ${language.dates.days}`;
        }
        ...
        if (daysDiff === 0) {
            ...
            dateString = `${result} ${language.dates.ago}`;
        }
                        ...
                        <div className="col-12 col-lg-3 p-3 d-flex flex-column justify-content-center">
                            ...
                            <div className="mt-2">
                                <a data-tooltip-id={props.data.previous_usernames?.length > 0 ? "reactTooltip" : ""}
                                   data-tooltip-content={`${language.user.top.previous}: ${props.data.previous_usernames?.join(', ')}`}
                            ...
                            <div className="level d-flex flex-row align-items-center gap-3 px-4">
                                <h6 className="p-0 m-0">{language.user.top.lvl} {props.data.statistics?.level.current}</h6>
                            ...
                            <div
                                ...
                                style={{fontSize: 14}}>
                                {language.user.top.joined} {formatDate(new Date(props.data.join_date))}
                            </div>
                        </div>
                        <div className="col-12 col-lg-2 p-3">
                            <div className="fs-1 d-flex flex-row align-items-center gap-1"
                                 data-tooltip-id="reactTooltip"
                                 data-tooltip-content={`${language.user.top.peakGlobal}: #${props.data?.rank_highest?.rank?.toLocaleString()}`}>
                            ...
                            <div>
                                <div className="fs-2 d-flex flex-row align-items-center gap-1">
                                    ...
                                    <div data-tooltip-id="reactTooltip"
                                         data-tooltip-content={`${language.user.top.peakCountry}: #${props.data.db_info?.country_rank?.length ?
                        ...
                        <div className="col-12 col-lg-2 p-3 d-flex flex-column gap-3">
                            <div>
                                <div className="d-flex flex-row align-items-center gap-2">
                                    ...
                                    <div>{language.user.top.rankedScore}:</div>
                                </div>
                                <div className="fs-5 ms-4"
                                     data-tooltip-id="reactTooltip"
                                     data-tooltip-content={`${language.user.scores.score}: ${props.data.statistics?.total_score.toLocaleString()}`}>
                            ...
                            <div>
                                <div className="d-flex flex-row align-items-center gap-2">
                                    <CloseIcon/>
                                    <div>{language.user.top.maxCombo}:</div>
                                </div>
                            ...
                            <div>
                                <div className="d-flex flex-row align-items-center gap-2">
                                    <AccessTimeIcon/>
                                    <div>{language.user.top.playTime}:</div>
                                </div>
                                <div className="fs-5 ms-4"
                            ...
                            <div>
                                <div className="d-flex flex-row align-items-center gap-2">
                                    <ReplayIcon/>
                                    <div>{language.user.top.playCount}:</div>
                                </div>
                                <div className="fs-5 ms-4">
